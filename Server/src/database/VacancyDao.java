...
public class VacancyDao {
	public List<Vacancy> listVacancies(boolean open, User user) {
		...
		Date date = null;
		String name = null, text = null, profile = null, userId = null, organisationName = null,  contactName = null;
		...
		try(Connection conn = DatabaseConnectionPool.getConnection()) {
			...
			if(open) {
				...
				statement = conn.prepareStatement("SELECT vacancy_id, vacancy_status, name, vacancy_date, notes, profile, organisation_organisation_id, user_user_id, " +
					"contact_contact_id FROM vacancy WHERE user_user_id LIKE ? AND vacancy_status LIKE ?");
				statement.setString(1, userType);
				statement.setObject(2, open);
			} else {
				// run this if looking for all vacancies
				statement = conn.prepareStatement("SELECT vacancy_id, vacancy_status, name, vacancy_date, notes, profile, organisation_organisation_id, user_user_id, " +
						"contact_contact_id FROM vacancy WHERE user_user_id LIKE ?");
				statement.setString(1, userType);
			}
			...
			while (rs.next()) {
				...
				PreparedStatement conStatement = conn.prepareStatement("SELECT first_name, surname FROM contact WHERE contact_id = ?");
				...
				Vacancy vacancy = new Vacancy(vacancyId, vacancyStatus, name, date, text, profile, organisationId, organisationName, userId, contactId, contactName);
